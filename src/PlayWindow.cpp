...
void PlayWindow::PlayWindow::setupUI() { //finished
    ...
    offsetFronm0ToLine = checkerLineHeight * perFrame * 1.0 / speed * 1.0 ;
...
void PlayWindow::showPauseMenu() {
    ...
    // Add buttons
    QPixmap continueimg, restartimg, exitimg;
    continueimg.load("./img/continue.png");
    restartimg.load("./img/restart.png");
    exitimg.load("./img/exit.png");
    continueButton = new ClickablePixmapItem(continueimg);
    continueButton -> setZValue(100);
    restartButton = new ClickablePixmapItem(restartimg);
    restartButton -> setZValue(100);
    exitButton = new ClickablePixmapItem(exitimg);
...
void PlayWindow::loadChart() {
    ...
    FILE* chartFile = fopen(chartSource.toStdString().c_str(), "r");
